...
public class UPbot {
     ...
     */
    public static void changeTranslation(String key, String newTranslation) {
        try {
            // make the change on the Wiki database
            MediaWikiBot b = initBot();
            SimpleArticle sa = b.readData("Translations:Gecko/" + key + "/" + LangInit.language.substring(0, 2).toLowerCase());
            String wikiCode = sa.getText();
            wikiCode = wikiCode.replace(LangInit.transMap.KeytoValue.get(key), newTranslation);
            sa.setText(wikiCode);
            b.writeContent(sa);
        } catch(Exception e) {
